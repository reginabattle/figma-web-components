...
2. **Styling Transparency**:
   Light DOM custom elements do not create style boundaries. Therefore, the core styling rules declared in `sds-button.css` naturally style the inner `<button>` element generated by the custom element. This eliminates the need for complex CSS bundling, stylesheet injection, or shadow parts interfaces.
...
2. **Environment Configuration**:
   - Set up `.storybook/preview.js` to dynamically load global design tokens (`styles/tokens.css`), component styles (`components/button/sds-button.css`), and register our Light DOM Web Component globally (`components/button/sds-button.js`).
